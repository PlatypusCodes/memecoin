...
let _candleCache = { bucketMs: 0, maxCandles: 0, len: 0, gen: -1, candles: [] };

function buildCandles(bucketMs, maxCandles) {
  if (!ticks.length) return [];
  // Cache key: bucketMs + maxCandles + ticks.length + _ticksGen.
  // _ticksGen catches reassignments (slice/rebuild) where length stays the same
  // but tick content changes — without it the cache returns stale candles after
  // a history rebuild or MAX_TICK_HISTORY trim.
  if (_candleCache.bucketMs === bucketMs &&
      _candleCache.maxCandles === maxCandles &&
      _candleCache.len === ticks.length &&
      _candleCache.gen === _ticksGen) {
  ...
  _candleCache = { bucketMs, maxCandles, len: ticks.length, gen: _ticksGen, candles };
  return candles;
}
...
// Reset Everything (coin history, transaction feed, leaderboard, every
// player's cash/holdings, and portfolio history) — a full game reset.
